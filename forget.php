...
<div class="section">
  <div class="main1">
    ...
    <form class="for-pass" action="<?php echo $_SERVER['PHP_SELF'];?>" method="POST">
    <div class="for-email">
        <label class="label-text">
          Email Address
          <span class="astris">*</span>
        </label>
        <div id="useremail" class="input-mail">
          <div class="la la-envelope input-icon"></div>
        <input type="email" name="mail" placeholder="Enter Email Address" autocomplete="off">
       </div>
    </div>
       <div id="username" class="for-user">
        <div class="la la-user input-icon"></div>
      <input type="text" name="user" placeholder="Enter User Name" autocomplete="off">
     </div>
      <div class="bott">
       <div class="btn-box">
    ...
    <?php
          $username = $conn->query("SELECT user_fullname FROM register")->fetchAll(PDO::FETCH_COLUMN);
          $useremail = $conn->query("SELECT user_email FROM register")->fetchAll(PDO::FETCH_COLUMN);
          $adminname = $conn->query("SELECT admin_name FROM admins")->fetchAll(PDO::FETCH_COLUMN);
          $adminemail = $conn->query("SELECT admin_email FROM admins")->fetchAll(PDO::FETCH_COLUMN);
          if( $_SERVER['REQUEST_METHOD'] == "POST"){
              if (empty($_POST['mail']) || empty($_POST['user'])){
                  echo "<script>alert('Please Enter Email And User Name ');
                  document.getElementById('username').setAttribute('style','border-color:red');
                  document.getElementById('useremail').setAttribute('style','border-color:red');
                  </script>";
              }
              else{
                  if( in_array($_POST['user'],$username) && in_array($_POST['mail'],$useremail) ){
                  ...
                  else if( in_array($_POST['user'],$adminname) && in_array($_POST['mail'],$adminemail) ){
                    $_SESSION['user'] = $_POST['user'];
                    $_SESSION['email'] = $_POST['mail'];
                    ...
                    echo "<p style =\"text-align :center;font-size:15px;color:red;\">Invalid Email or User Name</p>";
